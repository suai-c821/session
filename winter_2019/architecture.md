...
2. Шина PCI – стандартная шина для подключения периферийных устройств, среди которых звуковые карты, платы для захвата видео и т.д. Одной из основных возможностей является синхронный 32 или 64 разрядный обмен данными.
...
1. ACPI (Advanced Configuration and Power Interface) - общий интерфейс для обнаружения аппаратного обеспечения, управления питанием и конфигурации материнской платы и устройств. В ACPI 2.0 была добавлена поддержка 64-разрядных микропроцессоров, поддержка различных типов памяти, устройств PCI и PCI-X. В настоящий момент последней версией является ACPI 6.2a.

   Основной задачей ACPI - является обеспечить взаимодействие между операционной системой, аппаратным обеспечением и BIOS материнской платы.

2. SMBus (System Managment Bus) - последовательный протокол обмена данными для устройств питания, основан на шине I2C.
   ...
   Назначение SMBus:
   - Определение объёма и конфигурация памяти
   - Предоставление информации изготовителя
   - Сообщение системе номер модели и номер по каталогу устройства
   - Сообщение о различных типах ошибок
   - Отключение тактовой частоты на свободных разъёмах памяти
   - Определение пониженного напряжения батареи

## № 24 (COM-порт, IrDA и LPT. Основные теоретические сведения)

1. COM-порт (COMmunication port) - двунаправленный последовательный интерфейс, предназначенный для обмена байтовой информацией.

   Использовался для подключения терминала, позже для модема и мыши. Сейчас используется для соединения с источниками бесперебойного питания.
...
Технология Power Delivery - позволяет гибко управлять питанием через шину USB и обеспечивает передачу энергии до 100 Вт.

## № 26 (Шина FireWire, порт Bluetooth. Основные теоретические сведения)

1. FireWire (IEEE-1394) - последовательный интерфейс, используется для подключения цифровых видеокамер, внешних жёстких дисков и различных сетевых устройств. Для подключения устройств используется 6 жильный медный кабель или оптоволокно.

   Состав кабеля:

   - 2 провода под питание. Напряжение 8-40V. Ток 1,5A.

   - 4 провода - 2 экранированные витые пары для передачи информации. 
...
Внешние Запоминающие Устройства (ВЗУ) - устройства для хранения, чтения и записи информации. Основные свойства ВЗУ - энергонезависимость, отсутствие прямой связи с процессором.

Принципы:

1. Магнитный признак
    Основан на перемещении магнитных участков носителей относительно головки. Чтение и запись производится на треке.
...
2. Оптический признак
    Чтение и запись происходит при помощи лазера
...
3. Электрический признак
    Основан на пороговом эффекте в полупроводниковых средствах. Для изменения состояния памяти требуется некоторая электрическая энергия. Существует несколько видов данной памяти:
...
HDD (Hard Disk Drive) - энергонезависимое перезаписываемое запоминающее устройство.

Информация записывается на алюминиевые или керамические диски, покрытые ферромагнитным материалом. На шпинделе двигателя HDD может быть закреплено как один так и несколько дисков. В рабочем режиме, магнитные головки не касаются поверхности дисков, благодаря потоку воздуха. Расстояние между головкой и диском составляет 2-10nm. При отсутствии вращения, магнитная головка находится на шпинделе (парковочное место).
...
1. Разъёма питания
2. Конфигурационных перемычек
3. Интерфейсного разъёма
4. Постоянного магнита (динамический привод)
5. Катушки позиционирования (динамический привод)
6. Оси позиционера
7. Позиционера (коромысла)
8. Магнитной головки
9. Оси вращения дисков (шпинделя)
10. Магнитных дисков
11. Корпуса
12. Гермозоны
...
- CHS (Cylinder Head Sector) - система адресации сектора, как минимальной единицы хранения данных в HDD, основанная на использовании физических адресов геометрии диска.
- LBA (Logical Block Addressing) - способ нумерации секторов, не зависящий от внутренней физической архитектуры. Каждому сектору назначается свой ID.

Разметка:

- MBR - содержит загрузочный код и таблицу разделов
...
Блок электроники - отдельная съёмная плата находящаяся на HDD, которая включает в себя:
...
Оптические накопители - съёмные накопители информации, предназначенные для записи/чтения/хранения/распространения пользовательских данных.

CD диски состоят из поликарбонатной основы, отражающего слоя и защитного слоя. Углубление называются питами, а ровная отражающая поверхность - площадкой. Переход от площадки к питу воспринимается приёмником как бит. Источником луча является лазер.
...
- CD-R - диск однократной записи.
  ...
  В качестве регистрирующего слоя используется органический краситель. При записи происходит нагревание, после которого остаётся след на красителе.
...
DVD имеет уменьшенную питу (углубление) в два раза по сравнению с CD, а также уменьшенное расстояние между треками. Служебная информация стала занимать меньше места. Также из особенностей можно выделить: многослойность и двусторонность.
...
- По режимам:
  ...
  - Векторные - высвечивание точек в пределах знака-места. 
  - Текстовый режим - дисплей делится на отдельные знак-места.
- По цветам:
  - Монохромный (только одна лучевая трубка)
  - Цветные (3 луча (RGB))

![](architecture/crt_shadow_mask.png)

Теневая маска - пластина с отверстиями, которые по форме соответствуют расположению цветовых точек.
...
Принцип работы LCD основан на свойстве жидких кристаллов изменять (поворачивать) плоскость поляризации проходящего через них света пропорционально приложенному к ним напряжению. То есть изменяя величину напряжения можно управлять количеством пропускаемого света.

Матрицы:

- Пассивная матрица (LED) - электроды адресации находятся внутри стеклянных пластин. Напряжения у каждой пластины.
- Активная матрица (OLED) - каждая точка адресуется индивидуально, смена изображения построчная.
- TFT матрицы.
