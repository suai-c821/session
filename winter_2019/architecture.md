...
Прямой доступ к памяти (DMA) - это режим обмена данных между устройствами или между устройствами и памятью RAM без участия CPU. В результате увеличивается скорость передаёт информацию, так как данные не пересылаются в CPU и обратно

Принципы:

- CPU программирует DMA, устанавливая его регистры, затем процессор даёт команду устройства, чтобы та могла читать данные во виртуальном буфере.
- DMA посылает запрос HDD на чтение. После завершения записи, процесс повторяется, пока значение счётчика не = 0. После завершения цикла устройство инициализирует прерывание процессора. Это прерывание отвечает за завершение процесса передачи данных
...
1. Шина ISA - 8 или 16 разрядная шина ввода вывода IBM. Служит для подключения плат расширения.

2. Шина PCI – является стандартной шиной для подключения периферийных устройств. среди них можно отметить сетевые порталы, модемы, звуковые карты, платы для захвата видео.

   Основные возможности:

   - Синхронный 32 и 64 разрядный обмен данными

3. PCI-Express – последовательный интерфейс служит для графических карт используют параллельную передачу сигнала.

   Особенности:

   - Совместимость с существующими станциями PSI
   ...
   - Оптическая связь
   - Имеет встроенную системную синхронизацию и может быстро изменять частоту
