...
- Принцип последовательного программного управления: программа состоит из совокупности команд, которые выполняются ЦП последовательно друг за другом.
...
Аккумулятор - сохраняет результаты выполнения арифметических и логических комманд.
...
Система UMA (Uniform Memory Access) - это архитектура с общей памятью для многопроцессорных систем. В этой модели используется единственная память, к которой обращаются все процессоры представленной многопроцессорной системы. Каждый процессор имеет равное время доступа к памяти (задержка) и скорость доступа. Он может использовать либо одну шину, несколько шин.
...
Многопроцессорные системы с распределённой памятью не обеспечивают общего доступа ко всей имеющийся памяте (NORMA).

![](./architecture/multicomputer.jpg)

Каждый процессор системы может использоватьтолько свою локальную память, при этом доступ к данным на других ЦП необходимо явно выполнить передачу сообщений. Данный подход выполняется для МРР и кластеров.
...
Для паралленього усправления в алгоритмах решения задач достаточно выделить только крупные независимые части расчётов.
...
Демультиплексор (DMX) - передаёт сигнал с информационного входа на один из выходов - его номер равен десятичному эквиваленту двоичного кода на адрессных выходах.

![](./architecture/dmx.jpg)

Мультиплексор (MUX) - передача сигнала с одного из информационных входов, номер равен десятичному эквиваленту.

![](./architecture/mux.jpg)

# № 14 (Определение микропроцессора и МПС. Обобщённая структура микропроцессора.)

Микропроцессор – преогромно управляемое устройство постоянное на одной или нескольких бис и служит для обработки информации и управлением процессом этой обработки
...
Прямой доступ к памяти (DMA) - это режим обмена данных между устройствами или между устройствами и памятью ПАМ без участия ЦП. В результате увеличивается скорость передаёт информацию, так как данные не пересылаются в ЦП и обратно

Принципы:
ЦП программирует DMA, устанавливая его регитсры, затем процессор даёт комманду устройства, чтобы та могла читать данные во виртуальном буфере.

DMA посылает запрос ЖД на чтение. После завершения записи, процесс повторяется, пока значение счётчика не = 0. После завершения цикла устройство инициализирует прирывание процессора. Это прерывание отвечает за завершение процесса передачи данных
...
Чипсет - размещаемый на материнской плате набор микросхем, выполняющий роль связуещего компонента (моста), обеспечивающего взаимодействие ЦП с разными компонентами памяти, устройствами ввода и вывода, контроллерами и перефирию через системную шину.

Составляющие:
  1. Северный мост - отвечает за процессор, оперативную память и видеокарту.
  2. Южный мост - переферийные устройства.
...
В целом системная шина объединяет несколько магистралей: адреса данных служебную питания.
В конечном итоге подключение к разъёму процессора. Разрядность, шины данных в значительной мере определяется производительностью процессора.

Шина FSB

FSB – системная шина используется для связи с северным мостомFSB разработано компанией intel в первые использовано на базе процессора пентиум.

Шина HyperTrasport

Высокоскоростной последовательный интерфейс HyperTrasport предназначен для использования в качество внутренней шины ПК, в том числе для соединения процессора с прочими компонентами.

## №20 (Шины ISA, PCI, PCI-Express, AGP. Основные теоритические сведения.)

Шина ISA - 8 или 16 разрядная шина ввода вывода IBM. Служит для подключения плат расширения.

Шина PCI – pci является стандартной шиной для подключения периферийных устройств. среди них можно отметить сетевые порталы, модемы, звуковые карты, платы для захвата видео.

Основные возможности:

Синхронный 32 и 64 разрядный обмен данными

### Шина AGP

Ее отличия от PCI

Работа на тактовой частоте 66Мгц
...
Большой объём памяти не требуется тк видеопамять находится в общем адресном пространстве

### Шина PCIexpress

PCIexpress – последовательный интерфейс служит для графических карт используют параллельную передачу сигнала.

Особенности:

1. Совместимость с существующими станциями PSI

2. Физическое соединение осущ при помощи медных проводников

3. Имеет максимальную пропускную способность для каждого вывода

4. Оптическая связь

5. Имеет встроенную системную синхронизацию и может быстро изменять частоту

6. Низкое время зарядки
...
## № 27 (ВЗУ. Принципы записи и чтения.)
...
Магнитный - основан на перемещении многочисленных участков носителя относительно головки. Чтение и запись на дорожку. При записи вызываются изменнение ненамагнитических участков, а при чтении фиксирует изменения.
...
Оптические свойства участков носителя.

Либо это степень прозрачности, либо сохранения отражения. Запись с помощью лазера.

Плюсы:
1. Относительно невысокая плотность записи.
2. Долгосрочное хранение информации.

Минусы:
1. Сложный процесс перезаписи.
2. Невысокая скорость

### Электрический

Для изменения состояния ячейки памяти требуется электроэнергия.
...
Минусы:
1. Ограниченное количество циклов перезаписи

## № 29 (Виды адресации данных в накопителях на жёстких магнитных дисках. Сервоинформация)

ЖД делятся на отрезки - сектора. Совокупность секторов - кластеры. Цилиндр - совокупность дорожек, находящихся на равном расстоянии от центра диска по периметру. Есть 2 системы адрессации:
1. CHS (Цилиндр, головка, сектор)
2. LBD (Линейная система адрессации)
У каждого сектора свой номер.

Разметка ЖД - распределение ЖД на логические разделы.
...
Есть два метода: CHS и LBA

LBA - более простой способ нумерации секторов, не зависящий от внутренней физической архитектуры
CHS - системная адрессация сектора, как минимальной единицы хранится в данных в носителях на жёстких дисках, основанная на исполнения физических адрессов геометрии диска
